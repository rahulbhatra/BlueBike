...
ggp = ggplot(data = test) + 
  geom_line(aes(x = date, y = totalTrips), color = 'red') + 
  geom_line(aes(x = date, y = glmnet.pred), color = 'green')
ggp
...
```{r}
resamps <- resamples(list(glmnet = glmnet.mod, glmnet.pois = pois.mod))
ss <- summary(resamps)
