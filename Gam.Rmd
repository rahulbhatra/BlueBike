...
```{r}
trainX <- train[, c("temp", "humidity", "windspeed", "visibility", "feelslike", "uvindex")]
trainX
trainY <- train[ , c("totalTrips")]
trainY
  
rf.mod <- train(trainX = trainX,
                trainY = trainy,
                method = "rf",
                trControl = myTimeControl,
                tuneLength=tuneLength.num,
...
bestlam
pois.pred = predict(pois.mod$finalModel, newx)
dim(pois.pred)
pois.pred
